...
function createClientPromise() {
    if (!client)
        throw new Error("MONGODB_PROFILE_URI not configured");
    let lastErr;
    const maxAttempts = 3;
    const delayMs = 800;
...
if (uri)
    global.__mongoProfileClientPromise = clientPromise;
async function getProfileDb() {
    if (!clientPromise)
        throw new Error("Profiles DB not configured. Missing MONGODB_PROFILE_URI");
